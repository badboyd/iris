...
func initVips() {
	...
	if err := C.vips_initialize(); err != 0 {
		C.vips_shutdown()
		log.Fatal("unable to start vips!")
	}
	...
	if config.Image.JpegProgressive {
		cConf.JpegProgressive = C.int(1)
	}

	if config.Image.PngInterlaced {
		cConf.PngInterlaced = C.int(1)
	}

	cConf.WatermarkOpacity = C.double(config.Image.WatermarkOpacity)
...
func transformGif(ctx context.Context, img **C.VipsImage, po *processingOptions) error {
	...
	framesCount := minInt(imgHeight/frameHeight, config.Image.MaxGifFrames)
...
func processImage(ctx context.Context) ([]byte, context.CancelFunc, error) {
	...
	data := getImageDataBuffer(ctx).Bytes()
...
func vipsPrepareWatermark() error {
	...
	if C.vips_apply_opacity(watermark, &tmp, C.double(config.Image.WatermarkOpacity)) != 0 {
		return vipsError()
	}
...
func vipsSaveImage(img *C.VipsImage, imgtype imageType, quality int) ([]byte, context.CancelFunc, error) {
	...
	case imageTypePNG:
		if err = C.vips_pngsave_go(img, &ptr, &imgsize, cConf.PngInterlaced, 1); err != 0 {
			C.g_free_go(&ptr)
			log.Warn("Failed to save PNG; Trying not to embed icc profile")
